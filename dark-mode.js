/* ── Dark mode toggle ─────────────────────────────────────────
   Reads/writes data-theme="dark" on <html>.
   Preference is persisted in localStorage so it survives
   page navigation and browser restarts.
   ─────────────────────────────────────────────────────────── */

(function () {
  'use strict';

  const STORAGE_KEY = 'wander-theme';
  const html = document.documentElement;

  /* Apply theme as early as possible (called inline from <head>
     too, but this handles the module-script path as well). */
  function applyTheme(dark) {
    if (dark) {
      html.setAttribute('data-theme', 'dark');
    } else {
      html.removeAttribute('data-theme');
    }
  }

  /* Determine initial preference: stored value → OS preference */
  function getInitialDark() {
    const stored = localStorage.getItem(STORAGE_KEY);
    if (stored !== null) return stored === 'dark';
    return window.matchMedia('(prefers-color-scheme: dark)').matches;
  }

  /* Build the toggle button SVG icon */
  function buildIcon(dark) {
    if (dark) {
      /* Sun icon — clicking will switch to light */
      return `<svg width="18" height="18" viewBox="0 0 24 24" fill="none"
          aria-hidden="true"
          stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <circle cx="12" cy="12" r="5"/>
        <line x1="12" y1="1"  x2="12" y2="3"/>
        <line x1="12" y1="21" x2="12" y2="23"/>
        <line x1="4.22" y1="4.22"  x2="5.64" y2="5.64"/>
        <line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/>
        <line x1="1"  y1="12" x2="3"  y2="12"/>
        <line x1="21" y1="12" x2="23" y2="12"/>
        <line x1="4.22"  y1="19.78" x2="5.64" y2="18.36"/>
        <line x1="18.36" y1="5.64"  x2="19.78" y2="4.22"/>
      </svg>`;
    }
    /* Moon icon — clicking will switch to dark */
    return `<svg width="16" height="16" viewBox="0 0 24 24" fill="none"
        aria-hidden="true"
        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/>
    </svg>`;
  }

  function updateButton(btn, dark) {
    btn.innerHTML = buildIcon(dark);
    btn.setAttribute('aria-label', dark ? 'Switch to light mode' : 'Switch to dark mode');
    btn.setAttribute('title',      dark ? 'Switch to light mode' : 'Switch to dark mode');
  }

  /* Wire up every toggle button on the page */
  function initToggle() {
    let dark = getInitialDark();
    applyTheme(dark);

    const buttons = document.querySelectorAll('.dark-toggle');
    buttons.forEach(btn => {
      updateButton(btn, dark);
      btn.addEventListener('click', () => {
        dark = !dark;
        applyTheme(dark);
        localStorage.setItem(STORAGE_KEY, dark ? 'dark' : 'light');
        buttons.forEach(b => updateButton(b, dark));
      });
    });

    /* Respond to OS-level changes (e.g. system switches to dark) */
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', e => {
      /* Only follow OS if the user hasn't made an explicit choice */
      if (localStorage.getItem(STORAGE_KEY) === null) {
        dark = e.matches;
        applyTheme(dark);
        buttons.forEach(b => updateButton(b, dark));
      }
    });
  }

  /* Run after DOM is ready */
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initToggle);
  } else {
    initToggle();
  }
})();
